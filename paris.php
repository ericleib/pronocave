  ...
  if($phase['value_int']==0){
    ...
    $q = "DELETE FROM prono_paris
        WHERE id_pari='$id_pari[0]'
		   ...
		   OR id_pari='$id_pari[5]'";
    $r = mysql_query($q) or die(mysql_error());
    header("Location:paris.php#$poule");
  }
}

if(isset($_POST['set_poule_bet'])){
  $q = "SELECT value_int FROM prono_vars WHERE name='phase'";
  $r = mysql_query($q) or die(mysql_error());
  $phase = mysql_fetch_array($r);
  if($phase['value_int']==0){
...
<html>
<head>
       <title>Pronostics - Matchs de poules</title>
...
<tr><th colspan='4'>Poules (modifiable jusqu'au 10/06 15:00)</th></tr>
<tr><th colspan='4' style="color:red">Entrer les résultats poule par poule !</th></tr>
<?php
  ...
  while ($match = mysql_fetch_array($r_matchs)){
    ...
    if($cpt_poule==0){
      echo "<tr><form action='' method='post'>
 		    <td width='50' rowspan='6'>Poule ".$poule."</td><td align='right' style='border-right:0px'><a name=$poule></a>";
    ...
    if($cpt_poule==5){ // 5 = 0.5*N*(N-1) avec N= nombre d'équipes/poule
	  $cpt_poule=-1;
      echo "</form>";
    }
      elseif($cpt_poule==0){
