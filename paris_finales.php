...
function print_pari($n_paris,$pari,$id_team_A_m,$id_team_B_m,$done){
  ...
  if($done>3 && $id_team_A_m!=$id_team_A)
    echo "<font color='red'>".$nameA."</font>";
	else
	  echo $nameA;
  echo"<br>".$pari_A."<br>-<br>".$pari_B."<br>";
...
if(isset($_POST['send_bet'])){
  ...
  if($pari_A==$pari_B) header("Location:paris_finales.php?erreur=even#top");
  else{
...
if(isset($_POST['modify_bet'])){
  ...
  $test=0;
  if ($phase!='finale'){
    $q = "SELECT id_match FROM prono_paris WHERE id_user=$id_user";
...
if(isset($_GET['erreur']) && ($_GET['erreur'] == "ordre"))
    echo "<h1>Modifie tes pronos dans l'ordre !</h1>";
...
<br>
<a href='paris.php'>Matchs de poules</a>
<br>
<table width="600" border="1" cellspacing='0' align='center'>
<tr><th colspan='4'>Phases finales (modifiable jusqu'au 26/06 15:00)</th></tr>
...
<?php
  ...
  while($match=mysql_fetch_array($r_matchs)){
	...
	if($n_paris>=1){
	  ...
	  if($phase['value_int']<=1) print_form_mod($id_pari,'8emes');
	}
	else{
